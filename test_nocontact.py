import requests
from pager_slack_test import send_slack_message
...
def list_users_without_phone_number():
    ...
    for user in all_users:
        ...
        # Check if there is a contact method with label 'Work' or any other label for phone numbers
        has_phone = False
        for contact in contact_methods:
            # Check if 'label' contains 'phone' and 'address' contains the phone number
            #if 'phone' in contact.get("label", "").lower() and contact.get("address"):
